...
function TemplatePreviewCard({ t, onInsert }: { t: UITemplate; onInsert: (html: string, css: string) => void }) {
  ...
  const triggerHover = useCallback((enter: boolean) => {
    ...
    const el = iframe.contentDocument.querySelector("button, a, .bookmarkBtn-uv, .rainbow-hover-uv, .golden-button-uv, .custom-btn-uv, .skew-btn-uv, .blue-btn-uv, .line-hover-uv, .playstore-btn-uv, .btn-shine-uv") as HTMLElement | null;
    if (!el) return;
    if (enter) {
