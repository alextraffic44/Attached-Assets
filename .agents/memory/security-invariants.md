...
- Generated-site HTML (SEO machine + builder output) is served verbatim from a static host with NO framework auto-escaping, so every user/AI-derived value interpolated into that markup must be escaped/validated for its exact context before it ships.
- **Why:** the SEO setup form lets a user supply a project name, a target link, and a CTA label that flow straight into nav/footer/hero/CTA markup. Interpolating them raw was a real stored-XSS sink (`javascript:` hrefs, `"><script>` in labels/names). There are TWO CTA render sites — the homepage hero AND each generated article — and a fix to one silently leaves the other exploitable, so audit every render site, not just the one in the diff.
- **How to apply:** never interpolate a raw URL/string into `style="…"`, `href="…"`, a text node, or `<script>` in the builder functions. Route URLs through the URL validators (CSS-`url()` validator and the href validator both live in `seo-routes.ts` and reject non-http(s)/attribute-breaking input, degrading to a gradient / no-link), and route any text/attribute value through the HTML escaper. Sanitize names at the API boundary too, but still escape at render since stored data can predate the sanitizer. Pre-existing raw `siteTitle`/`siteDescription`/AI-HTML interpolation is currently accepted as owner-only self-XSS (each published site contains only its owner's content); revisit if SEO pages ever become untrusted multi-tenant content.

## Payment crediting must be exactly-once (two racing paths)
- There are TWO paths that credit a paid 1payment order: the async webhook AND the client-polled `POST /api/payments/check-status`. Both can fire for the same order. Crediting MUST be claim-first: insert the `creditTransactions` idempotency row (`payment_<orderId>`) with `.onConflictDoNothing().returning()` FIRST, and call `storage.addCredits` ONLY when a row was actually inserted.
- **Why:** `updatePaymentOrderStatus(...'paid')` is unconditional (no claim semantics), so the `order.status !== 'paid'` guard does NOT serialize the two paths — both could read not-paid and both call addCredits → double credit. The unique idempotencyKey silently swallows the second transaction ROW but does NOT stop a second balance UPDATE. Gating addCredits on the insert result is what makes it exactly-once. Never revert to read-then-write `updateUserCredits(id, user.credits + n)`; use atomic `addCredits` (SQL `credits = credits + n`).
- **Residual (accepted):** if addCredits throws after the order is marked paid, the credit is lost with no retry — pre-existing, low priority.

## Publish plan-limit slot semantics
- `publishStatus` values carry billing meaning: `suspended` = billing suspension (insufficient balance) and MUST keep occupying a plan slot (re-published on top-up). Voluntary unpublish MUST set `draft` (frees the slot). Never make manual unpublish set `suspended`, or a bronze (1-site) user who takes a site down can never publish another.
- The per-plan publish limit (`PLAN_PUBLISH_LIMITS`, bronze=1/silver=2/gold=3/platinum=5/free=0) counts other projects in `published` + `publishing` + `suspended`. Include `publishing` (and treat it as `alreadyLive` for the same project) or two concurrent/in-flight publishes bypass the limit during the multi-second deploy window.

## IDN / .рф domain handling
- Custom-domain input must be normalized with `domainToASCII` (node:url) to punycode BEFORE validation/storage/Yandex calls, and the validation regex must allow `xn--` TLD labels (`(?:[a-z]{2,}|xn--[a-z0-9-]{2,})`). A pure-letters TLD regex silently rejects `.рф` (xn--p1ai) — a real defect for a RU-market product. Yandex CDN + Certificate Manager need the ASCII/punycode form anyway.
