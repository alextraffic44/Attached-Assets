...
- **AI Code (new site)**: Claude Opus 4.7 via KIE API (KIE_API_KEY env var, POST https://api.kie.ai/claude/v1/messages, Anthropic-style streaming SSE with `content_block_delta` events, manual conversation history)
- **AI Code (edit)**: Claude Opus 4.7 via KIE API for editing with SEARCH/REPLACE diff patches (falls back to full HTML if needed); base64 images stripped before sending to reduce context size
- **Design-to-Code (Mockup Mode)**: Toggle in chat when images attached — sends specialized prompt to Claude Opus 4.7 to analyze screenshot/mockup and recreate exact layout as HTML/CSS/JS (not embed as img)
- **Two-Step Mockup Analysis**: Step 1 extracts detailed JSON design spec (colors, typography, layout, sections, effects) via separate Claude sync call; Step 2 uses that structured analysis to generate pixel-perfect code
- **AI Enhance**: Claude Opus 4.7 via KIE API for prompt enhancement (5 tokens)
